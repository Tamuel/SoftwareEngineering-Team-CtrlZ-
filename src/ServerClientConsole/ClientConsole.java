 ...
 * 
 * 이 Class 는 Display() 메소드를 이용하여 채팅 UI를 제공한다. 주의: 여기있는 몇몇 코드들은 ServerConsole 에
 * Clone 으로 있다.
 * 
...
public class ClientConsole implements ChatIF // ChatIF 인터페이스 호출
{
	...
	/**
	 * The default port to connect on. DEFAULT_PORT : 포트 번호, 이 번호로 커넥트
	 */
	...
	/**
	 * The instance of the client that created this ConsoleChat. ChatClient 객체
	 * 생성
	 */
	 ...
	 * 
	 * 이 메소드는 console 창에 입력할 때까지 대기한다. console 창에서의 입력 client의 message handler로
	 * 보낸다.
	 */
	public void accept() {
		try {
			...
			while (true) // 무한 반복하면서 console 입력을 받아들임.
			{
				...
				if (!message.startsWith("#")) {
					...
					if (client.isConnected()) {
						...
						client = new Client(client.getHost(), client.getPort(),
								this);
					}
				} else if (message.equals("#logoff")) {
					client.quit();
				} else if (message.length() >= 8
						&& message.substring(0, 8).equals("#sethost")) {
					if (client.isConnected()) {
						System.err.println("logoff 해주시기 바랍니다.");
					} else if (message.contains("<") && message.contains(">")) {
						client.setHost(message.substring(
								message.indexOf('<') + 1, message.indexOf('>')));
					} else
						System.out.println("인자를 넣어 주시기 바랍니다.");
				} else if (message.length() >= 8
						&& message.substring(0, 8).equals("#setport")) {
					if (client.isConnected()) {
						System.err.println("logoff 해주시기 바랍니다.");
					} else if (message.contains("<") && message.contains(">")) {
						client.setPort(Integer.parseInt(message.substring(
								message.indexOf('<') + 1, message.indexOf('>'))));
					} else
						System.out.println("인자를 넣어 주시기 바랍니다.");
