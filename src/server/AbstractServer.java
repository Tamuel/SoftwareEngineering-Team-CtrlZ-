 ...
 * a new <code> ConnectionToClient </code> instance which runs as a thread. When
 * a client is thus connected to the server, the two programs can then exchange
 * <code> Object </code> instances.
 * <p>
 * 
 * 이 Class 는 client 에서 시도 하는 connection 을 기다리는 스레드를 포함한다. 만약 connection이 성사되면,
 * 스레드로 실행되는 새로운 ConnectionToCilent 객체를 만든다. Client가 서버에 연결되면, 두 프로그램은 Object
 * instances 들을 교환 가능하다.
 * 
 ...
 * framework, and several hook methods are also available
 * <p>
 *
...
public abstract class AbstractServer implements Runnable {
	 ...
	 * default.
	 * 
	 * 서버는 connection을 하는 중 시간 초과를 계산한다. 시간이 초과되면, 서버는 서버를 멈춰야 하는지 체크하고, 아니라면 계속
	 * 연결을 시도한다. 초기치는 1/2초 이다.
	 */
	 ...
	 * 
	 * 서버에 연결하기 위해 대기할 수 있는 클라이언트의 수다. 10명이 기본이다.
	 */
	 ...
	 * 
	 * 클라이언트 스레드와 연결된 스레드 집합이다. 스레드 그룹의 각 멤버는 ConnectionToClient 이다.
	 */
	 ...
	 * 
	 * 새로운 클라이언트를 받아들이기 위한 스레드를 시작한다. 서버가 이미 리스닝 모드이면, 이 콜은 효과가 없다.
	 *
	 ...
	 * exception thrown while closing a client is ignored. If one wishes to
	 * catch these exceptions, then clients should be individually closed before
	 * calling this method. The method also stops listening if this thread is
	 * running. If the server is already closed, this call has no effect.
	 *
	 * 서버 소켓을 닫고, 모든 클라이언트와의 연결을 끊는다. 클라이언트를 닫는게 승인 받지 못하면 예외사항을 throw 한다. 이 예외
	 * 사항을 catch 하고 싶으면, 이 메소드를 이용하기 전에 각각 클라이언트를 닫아줘야 한다. 이 메소드는 실행중인 스레드의 리스닝도
	 * 멈추게 한다. 이미 서버가 닫혀있으면 아무 효과도 없다.
	 *
	 * @exception IOException
	...
	/**
	 * Sends a message to every client connected to the server. This is merely a
	 * utility; a subclass may want to do some checks before actually sending
	 * messages to all clients. This method can be overriden, but if so it
	 * should still perform the general function of sending to all clients,
	 * perhaps after some kind of filtering is done. Any exception thrown while
	 * sending the message to a particular client is ignored.
	 * 
	 * 서버에 연결된 모든 클라이언트들에게 메세지를 보낸다. Subclass에서 메세지를 모든 클라이언트들에게 보내기 전에 몇몇 요소들을
	 * 체크해보고 싶으면 오버라이드 가능하다.하지만 필터링을 따로 설정하지 않는다면 이 역시 모든 클라이언트들에게 메소드가 실행된다.
	 *
	 * @param msg
	 ...
	 * after this array is obtained, some clients in this might disconnect. New
	 * clients can also connect, these later will not appear in the array.
	 *
	 * 존재하는 클리이언트 connection 들을 배열로 리턴해준다. 이 메소드는 각각의 connection 에 뭔가를 할 수 있도록
	 * concrete subclass에 implement할 수 있다.
	 * 
	 * 이 배열이 생성되면 몇몇 클라이언트들이 끊길 수 있음을 기억해라. 새로운 클라이언트가 연결할 수 있지만, 배열에는 나타나지 않는다.
	 *
	 ...
	 * 
	 * OS에서 connection을 기다리는 스레드의 최대치를 정해줄 수 있다. 기본값은 20이다.
	 * 
	 *
	 * @param backlog
	 ...
	 * called. 클라이언트들의 연결을 허용하는 리스닝 스레드를 실행시킨다. call 할 필요는 없다.
	 */
	...
	/**
	 * Hook method called when the server stops accepting connections. The
	 * default implementation does nothing. This method may be overriden by
	 * subclasses.
	 */
	...
	/**
	 * Hook method called when the server is clased. The default implementation
	 * does nothing. This method may be overriden by subclasses. When the server
	 * is closed while still listening, serverStopped() will also be called.
	 */
	 ...
	 * called by a synchronized method so it is also implcitly synchronized.
	 *
	 * 클라이언트에서 서버로 보내는 메세지를 핸들링 한다. 이 메소드는 무조건! subclass 에서 implement 되어야 한다.
	 ...
	 */
	protected abstract void handleMessageFromClient(Object msg,
	 ...
	 * method of <code>ConnectionToClient</code> instances that are watching for
	 ...
	 *            the connection connected to the client that sent the message.
	 */
	final synchronized void receiveMessageFromClient(Object msg,
			ConnectionToClient client) {
